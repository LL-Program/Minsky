...
class FileIconProvider(QFileIconProvider):
    def icon(self, parameter):
        self.suffixList = ["py","c", "cpp", "todo", "exe", "svg"]
        current_dir =os.path.dirname(os.path.realpath(__file__))
        if isinstance(parameter, QFileInfo):
            info = parameter
            if info.isDir():
                try:
                    icon = QIcon(os.path.join(current_dir, os.path.join(sys.path[0],  f"UI/icons/fileicons/folder-home.svg")))
                    icon.addFile(os.path.join(current_dir, os.path.join(sys.path[0],  f"UI/icons/fileicons/folder-api.svg")), state = icon.On)
                    return icon
                except FileNotFoundError:
                    pass
            if info.suffix() in self.suffixList:
                try:
                    return QIcon(os.path.join(current_dir, os.path.join(sys.path[0],  f"UI/icons/fileicons/{info.suffix()}.svg")))
                except FileNotFoundError:
                    pass 
            else:
                try:
                    return QIcon(os.path.join(current_dir, os.path.join(sys.path[0],  f"UI/icons/fileicons/url.svg")))
                except FileNotFoundError:
                    pass
...
class FileManager(QTreeView):
    ...
    def show_dialog(self, title, msg) -> int:
        ...
        dialog.setWindowIcon(QIcon("UI/icons/close-icon.svg"))
        dialog.setText(msg)
